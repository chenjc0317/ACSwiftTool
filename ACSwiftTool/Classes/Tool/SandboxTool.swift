...
public extension SwiftTool{
    ...
    fileprivate static func getCacheFilePath(url: String, subPath:String?) -> String {
        ...
        //check路径
        let cacheFileNameString: String = "URL:\(url) AppVersion:\(SwiftTool.versionS)"
    ...
    fileprivate static func checkDirectory(_ path: String) {
        ...
        if !fileManager.fileExists(atPath: path, isDirectory: &isDir) {
            SwiftTool.createBaseDirectoryAtPath(path)
        } else {
            if !isDir.boolValue {
                do {
                    ...
                    SwiftTool.log("创建缓存文件夹失败，error - [\(error)]")
                }
            }
        }
    }
    ...
    static func createBaseDirectoryAtPath(_ path: String) {
        do {
            ...
            SwiftTool.log("创建文件夹失败！error[\(error)]")
        }
    }
    /// 设置不备份
    static func addDoNotBackupAttribute(_ path: String) {
        ...
        do {
            ...
            SwiftTool.log("设置不备份失败,error[\(error)]")
        }
    }
}
